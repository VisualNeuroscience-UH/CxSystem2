...
class TestConfigurationExecutor:
    ...
    def test_definition_lines_idx(self):
        ...
        assert all(
            CM.anat_and_sys_conf_df.loc[:, 0][
                CM.anat_and_sys_conf_df.loc[:, 0] == "row_type"
            ].index
            == np.array([0, 2, 5, 12])
        )
    ...
    def test_relay(self):
        ...
        assert len(CM.customized_neurons_list[1].keys()) == 20
        assert len(CM.customized_neurons_list[2].keys()) == 20
    ...
    def test_neuron_group(self):
        ...
        """
        assert all(
            isinstance(x, np.complex128)
            for x in CM.customized_neurons_list[0]["z_positions"]
        )
        assert all(
            isinstance(x, np.complex128)
            for x in CM.customized_neurons_list[0]["w_positions"]
        )
    ...
    def test_synapse(self):
        ...
        assert isinstance(
            CM.customized_synapses_list[0]["equation"],
...
class TestPhysiologyReference:
    ...
    def test_neuron_reference_init(self):
        assert all(
            np.round(CM.customized_neurons_list[0]["z_positions"], decimals=2)
            == np.round(
                list(
                    map(
                        lambda x: np.e ** (x / 17) - 1,
                        CM.customized_neurons_list[0]["w_positions"],
                    )
                ),
                decimals=2,
            )
        ...
        assert all(
            np.round(CM.customized_neurons_list[1]["z_positions"], decimals=2)
            == np.round(
                list(
                    map(
                        lambda x: np.e ** (x / 17) - 1,
                        CM.customized_neurons_list[1]["w_positions"],
                    )
                ),
                decimals=2,
            )
        ...
        assert all(
            np.round(CM.customized_neurons_list[2]["z_positions"], decimals=2)
            == np.round(
                list(
                    map(
                        lambda x: np.e ** (x / 17) - 1,
                        CM.customized_neurons_list[2]["w_positions"],
                    )
                ),
                decimals=2,
            )
    ...
    def test_PC1(self):
        ...
        assert "-ge_soma/tau_e" in str(
            CM.customized_neurons_list[1]["equation"].eq_expressions
        )
        assert "vm_a2" in str(CM.customized_neurons_list[1]["equation"].eq_expressions)

    def test_PC2(self):
        """Testing one part of equation in soma and one in apical dendrite"""
        assert "PC2" in CM.customized_neurons_list[2]["object_name"]
        assert "-ge_soma/tau_e" in str(
            CM.customized_neurons_list[2]["equation"].eq_expressions
        )
        assert "vm_a2" in str(CM.customized_neurons_list[2]["equation"].eq_expressions)

    def test_SS(self):
        """Testing one part of equation"""
        assert "SS" in CM.customized_neurons_list[3]["object_name"]
        assert "-ge_soma/tau_e" in str(
            CM.customized_neurons_list[3]["equation"].eq_expressions
        )
    ...
    def test_BC(self):
        ...
        assert "-ge_soma/tau_e" in str(
            CM.customized_neurons_list[4]["equation"].eq_expressions
        )
...
def get_spike_data():
    ...
    new_output_name = [
        item
        for item in os.listdir(CM.workspace.get_simulation_folder_as_posix())
        if "_results_" in item
    ]  # Assuming just one outputfile in this folder
    new_output_fullpath = path.joinpath(
        CM.workspace.get_simulation_folder_as_posix(), new_output_name[0]
    )
...
def test_outputfile(cxsystem_run_fixture):
    ...
    assert os.access(
        CM.workspace.get_simulation_folder().joinpath(outputfilelist[0]), os.W_OK
    )
