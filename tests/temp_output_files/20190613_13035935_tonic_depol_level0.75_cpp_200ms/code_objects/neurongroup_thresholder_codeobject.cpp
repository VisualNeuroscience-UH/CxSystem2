...
const int _num_spikespace = 61;
 ...
 int32_t* __restrict  _ptr_array_neurongroup__spikespace = _array_neurongroup__spikespace;
    ...
    long _count = 0;
    for(int _idx=0; _idx<60; _idx++)
    {
        const int _vectorisation_idx = _idx;
                
        const double emit_spike = _ptr_array_neurongroup_emit_spike[_idx];
        const char _cond = emit_spike >= 1;

        if(_cond) {
            _ptr_array_neurongroup__spikespace[_count++] = _idx;
        }
    }
    _ptr_array_neurongroup__spikespace[60] = _count;
