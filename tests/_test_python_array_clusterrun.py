...
anatomy_and_system_config = path.joinpath(
    "tests", "config_files", "pytest_Anatomy_config_cluster.csv"
).as_posix()
physiology_config = path.joinpath(
    "tests", "config_files", "pytest_Physiology_config_cluster.csv"
).as_posix()
...
cluster_simulation_path = Path(
    workspace_folder.joinpath("cluster_run" + simulation_suffix)
)
cluster_download_path = cluster_simulation_path.joinpath("downloads")
cluster_metadata_path = cluster_simulation_path.joinpath(
    "cluster_metadata{}.pkl".format(simulation_suffix)
)
...
def get_spike_data():
    ...
    new_output_name = [
        item
        for item in os.listdir(cluster_download_path.as_posix())
    ...
    assert (
        len(
            [
                ...
                if os.access(cluster_download_path.joinpath(item), os.W_OK)
            ]
        )
        == 6
    )
...
def test_spikecount_10percent_tolerance(cxsystem_run_fixture3, capsys, get_spike_data):
...
def test_spikecount_report(cxsystem_run_fixture3, capsys, get_spike_data):
    spikes_all, new_spikes_all = get_spike_data
    keys = list(spikes_all.keys())  # dict_keys is not indexable directly
...
def test_spiketiming_report(cxsystem_run_fixture3, capsys, get_spike_data):
    spikes_all, new_spikes_all = get_spike_data
    keys = list(spikes_all.keys())  # dict_keys is not indexable directly
