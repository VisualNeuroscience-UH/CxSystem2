...
class Stimuli:
    ...
    def initialize_inputs(self, freq):
        ...
        try:
            ...
            sparse_stimulus = np.tile(
                np.zeros_like(dense_stimulus), (1, soa_in_n_frames)
            )
        ...
        self.i_patterns[len(self.i_patterns)] = (
            frames.values * self.factor
        )  # These must be final firing rates
        _all_stim = np.squeeze(_V1_mats["stimulus"])
        if len(_all_stim.shape) == 2:
            slash_indices = [
                idx for idx, ltr in enumerate(self.input_mat_path) if ltr == "/"
            ]
            print(
                " -  One video stimulus found in file "
    ...
    def calculate_input_seqs(self):
        ...
        b2.set_device(
            ...
            directory=os.path.join(
                self.output_folder, "Input_cpp_run" + self.output_file_suffix
            ),
        )
        ...
        self.save_input_sequence(
            spikemons,
            os.path.join(self.output_folder, "input" + self.output_file_suffix),
        )
        shutil.rmtree(
            os.path.join(self.output_folder, "Input_cpp_run" + self.output_file_suffix)
        )
    ...
    def load_input_seq(self, input_spike_file_location):
        ...
        if os.path.isfile(input_spike_file_location):
            ...
            input_spike_file_location = os.path.join(
                input_spike_file_location,
                "input" + self.output_file_suffix + self.output_file_extension,
            )
        self.loaded_data = load_from_file(input_spike_file_location)
        new_spikes = self.loaded_data["spikes_0"]
        neuron_positions_in_cortex = io.loadmat(
            self.input_mat_path, variable_names="w_coord"
        )
