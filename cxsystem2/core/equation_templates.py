...
class EquationHelper:
    ...
    default_dendrite_strings = {
        "I_TONIC": "",
        "TONIC_EQ": "",
        "VM_NOISE": "",
        "BRIAN2_FLAGS": "",
    }  # Be careful! "Unless refractory" in dendrites will not cause an error, but behavior is WRONG
    ...
    NeuronModels["EIF_PC"] = {
        "soma": {
            "I_NEURON_MODEL": "gL*(EL-vm) + gL * DeltaT * exp((vm-VT) / DeltaT) + I_dendr",
            "NEURON_MODEL_EQ": "",
        },
    ...
    NeuronModels["ADEX"] = {
        "I_NEURON_MODEL": "gL*(EL-vm) + gL * DeltaT * exp((vm-VT) / DeltaT) - w",
        "NEURON_MODEL_EQ": "dw/dt = (-a*(EL-vm)-w)/tau_w : amp",
    }
    NeuronModels["ADEX_PC"] = dict()
    NeuronModels["ADEX_PC"] = {
        "soma": {
            "I_NEURON_MODEL": "gL*(EL-vm) + gL * DeltaT * exp((vm-VT) / DeltaT) + I_dendr",
            "NEURON_MODEL_EQ": "",
        },
    ...
    SynapticExcInhModels["SIMPLE_E"] = {
        "I_SYNAPTIC_EXC": "ge * (Ee - vm)",
        "SYNAPTIC_EXC_EQ": "dge/dt = -ge/tau_e : siemens",
    }
    SynapticExcInhModels["SIMPLE_I"] = {
        "I_SYNAPTIC_INH": "gi * (Ei - vm)",
    ...
    SynapticExcInhModels["E_ALPHA"] = {
        "I_SYNAPTIC_EXC": "gealpha * (Ee - vm)",
    ...
    SynapticExcInhModels["I_ALPHA"] = {
        "I_SYNAPTIC_INH": "gialpha * (Ei - vm)",
    ...
    SynapticExcInhModels["E_ALPHA_NONSCALED"] = {
        "I_SYNAPTIC_EXC": "gealpha * (Ee - vm)",
        "SYNAPTIC_EXC_EQ": """dge/dt = -ge/tau_e_alpha : siemens
    ...
    SynapticExcInhModels["I_ALPHA_NONSCALED"] = {
        "I_SYNAPTIC_INH": "gialpha * (Ei - vm)",
        "SYNAPTIC_INH_EQ": """dgi/dt = -gi/tau_i_alpha : siemens
    ...
    SynapticExcInhModels["E_ALPHA_NMDA"] = {
        "I_SYNAPTIC_EXC": "gealpha * (Ee - vm) + g_nmda_alpha*(E_nmda - vm)",
        "SYNAPTIC_EXC_EQ": """dge/dt = -ge/tau_e_alpha : siemens
                                                                dgealpha1/dt = (ge-gealpha1)/tau_e_alpha : siemens
                                                                gealpha = scaling_alpha*exp(1)*gealpha1 : siemens
    ...
    SynapticExcInhModels["I_ALPHA_GABAB"] = {
        "I_SYNAPTIC_INH": "gialpha * (Ei - vm) + g_gabab_alpha*(E_gabab - vm)",
        "SYNAPTIC_INH_EQ": """dgi/dt = -gi/tau_i_alpha : siemens
                                                    dgialpha1/dt = (gi-gialpha1)/tau_i_alpha : siemens
                                                    gialpha = scaling_alpha*exp(1)*gialpha1 : siemens
    ...
    SynapticExcInhModels["SIMPLE_E_NMDA"] = {
        "I_SYNAPTIC_EXC": "ge * (Ee - vm) + scaling_nmda*g_nmda*B*(E_nmda - vm)",
        "SYNAPTIC_EXC_EQ": """dge/dt = -ge/tau_e : siemens
                                                    dg_nmda/dt = -g_nmda/tau_decay_nmda : siemens
                                                    B = (1/(1+exp(-62*(vm/volt))*(1/3.57))) : 1
                                                 """,
    }
    SynapticExcInhModels["SIMPLE_I_GABAB"] = {
        "I_SYNAPTIC_INH": "gi * (Ei - vm) + scaling_gabab*g_gabab*(E_gabab - vm)",
        "SYNAPTIC_INH_EQ": """dgi/dt = -gi/tau_i : siemens
                                                     dg_gabab/dt = -g_gabab/tau_decay_gabab : siemens
                                                  """,
    }
    ...
    SynapticExcInhModels["AMPA_NMDA"] = {
        "I_SYNAPTIC_EXC": "g_ampa*(E_ampa - vm) + scaling_nmda*g_nmda*B*(E_nmda - vm)",
    ...
    SynapticExcInhModels["GABAA_GABAB"] = {
        "I_SYNAPTIC_INH": "g_gabaa*(E_gabaa - vm) + scaling_gabab*g_gabab*(E_gabab - vm)",
    ...
    SynapticExcInhModels["AMPA_NMDA_BIEXP"] = {
        "I_SYNAPTIC_EXC": "g_ampa_alpha*(E_ampa - vm) + g_nmda_alpha*(E_nmda - vm)",
        "SYNAPTIC_EXC_EQ": """dg_ampa/dt = -g_ampa/tau_decay_ampa : siemens
                                                        dg_nmda/dt = -g_nmda/tau_decay_nmda : siemens
    ...
    SynapticExcInhModels["GABAA_GABAB_BIEXP"] = {
        "I_SYNAPTIC_INH": "g_gabaa_alpha*(E_gabaa - vm) + g_gabab_alpha*(E_gabab - vm)",
        "SYNAPTIC_INH_EQ": """dg_gabaa/dt = -g_gabaa/tau_decay_gabaa : siemens
                                                  dg_gabab/dt = -g_gabab/tau_decay_gabab : siemens
    ...
    def __init__(
        ...
        if is_pyramidal is True:
            assert self.compartment_type in [
                "soma",
                "dendrite",
    ...
    def get_membrane_equation(self, return_string=False):
        ...
        if return_string is True:
            ...
            substitutables = {
                k: k + "_" + self.compartment for k in self.comp_specific_vars
            }
    ...
    def get_dict(self, base_dict=None, specific_compartment="XX"):
        ...
        substitutables = {
            k: k + "_" + specific_compartment for k in self.comp_specific_vars
        }
