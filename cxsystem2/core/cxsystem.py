...
class CxSystem:
    ...
    def __init__(
        ...
        * save_data: The save_data() object for saving the final data.
        ...
        self.min_distance = 0
        self.init_vms = 0
        # if there is at least one connection to save, this flag will be set to 1
        ...
        if params_indices[0].size > 1:
            for row_idx in params_indices[0][1:]:
                ...
                new_anat_and_sys_conf_df = pd.concat(
                    [self.anat_and_sys_conf_df, empty_dataframe], axis=1
                )

                new_anat_and_sys_conf_df.iloc[params_indices[0][0] - 1] = (
                    self.anat_and_sys_conf_df.iloc[params_indices[0][0] - 1]
                    .dropna()
                    .append(self.anat_and_sys_conf_df.iloc[row_idx - 1][1:])
                    .dropna()
                    .reset_index(drop=True)
                )
                new_anat_and_sys_conf_df.iloc[params_indices[0][0]] = (
                    self.anat_and_sys_conf_df.iloc[params_indices[0][0]]
                    .dropna()
                    .append(self.anat_and_sys_conf_df.iloc[row_idx][1:])
                    .dropna()
                    .reset_index(drop=True)
                )
        ...
        self.array_run = 0
        try:
            self.numerical_integration_method = parameter_finder(
                self.anat_and_sys_conf_df, "integration"
            )
        except NameError:
            self.numerical_integration_method = "euler"
        print(
            " -  The system is running with %s integration method"
            % self.numerical_integration_method
        )

        check_array_run_anatomy = self.anat_and_sys_conf_df.map(
            lambda x: True if ("|" in str(x) or "&" in str(x)) else False
        )
        check_array_run_physiology = self.physio_config_df.map(
            lambda x: True if ("|" in str(x) or "&" in str(x)) else False
        )
        try:
        ...
        if (
            np.any(check_array_run_anatomy)
            or np.any(check_array_run_physiology)
            or (trials_per_config > 1 and not instantiated_from_array_run)
        ):
            self.set_workspace(
                parameter_finder(self.anat_and_sys_conf_df, "workspace_path"),
                self.suffix,
            )
            self.workspace.create_simulation(
                parameter_finder(self.anat_and_sys_conf_df, "simulation_title")
            )
            suffix = self.suffix
            tmp_folder_path = self.workspace.get_workspace_folder().joinpath(
                ".tmp" + suffix
            )
            # if not tmp_folder_path.is_dir():
            os.makedirs(tmp_folder_path.as_posix(), exist_ok=True)
            tmp_anat_path = tmp_folder_path.joinpath(
                "tmp_anat" + self.suffix + ".csv"
            ).as_posix()
            tmp_physio_path = tmp_folder_path.joinpath(
                "tmp_phys" + self.suffix + ".csv"
            ).as_posix()
        ...
        for value_line_title in order_of_lines:
            for def_idx in definition_lines_idx:
                if value_line_title in self.anat_and_sys_conf_df.loc[def_idx + 1, 0]:
                    self.current_parameters_s = self.anat_and_sys_conf_df.loc[
                        def_idx, 1:
                    ].dropna()
                    self.current_parameters_s = self.current_parameters_s[
                        ~self.current_parameters_s.str.contains("#")
                    ]
                    self.current_parameters_s_orig_len = len(self.current_parameters_s)
                    try:
                        next_def_line_idx = definition_lines_idx[
                            definition_lines_idx.tolist().index(def_idx) + 1
                        ].item()
                    except IndexError:
                    ...
                    for self.value_line_idx in range(def_idx + 1, next_def_line_idx):
                        if (
                            ...
                            if (
                                ...
                                in list(self.parameter_to_method_mapping.keys())
                                and self.anat_and_sys_conf_df.loc[
                                    self.value_line_idx, 0
                                ][0]
                                != "#"
                            ):
                                self.current_parameters_s = (
                                    self.anat_and_sys_conf_df.loc[def_idx, 1:].dropna()
                                )
                                self.current_parameters_s = self.current_parameters_s[
                                    ~self.current_parameters_s.str.contains("#")
                                ]
                                self.current_values_s = self.anat_and_sys_conf_df.loc[
                                    self.value_line_idx, self.current_parameters_s.index
                                ].dropna()
                                self.parameter_to_method_mapping[
                                    self.anat_and_sys_conf_df.loc[
                                        self.value_line_idx, 0
                                    ]
                                ][1]()
                    break
    ...
    def create_simulation(self, *args):
        ...
        self.workspace.results["runtime"] = self.runtime
    ...
    def integration(self, *args):
        ...
        assert self.numerical_integration_method in [
            "exact",
            "exponential_euler",
            "euler",
            "rk2",
    ...
    def set_runtime_parameters(self):
        if not np.any(self.current_parameters_s.str.contains("runtime")):
            print(
                " -  The parameter 'workspace_path' is not defined in the configuration file. The default workspace is ~/CxSystem"
            )
        ...
        if not np.any(self.current_parameters_s.str.contains("compression_method")):
            ...
            print(
                " -  compression_method is not defined in the configuration file. Default compression method is gzip"
            )
        ...
        options_with_priority = [
            it
            for it in self.parameter_to_method_mapping
            if not np.isnan(self.parameter_to_method_mapping[it][0])
        ]
        parameters_to_set_prioritized = [
            it
            for priority_idx in range(len(options_with_priority))
            for it in self.parameter_to_method_mapping
            if self.parameter_to_method_mapping[it][0] == priority_idx
        ]
        for correct_parameter_to_set in parameters_to_set_prioritized:
            for ParamIdx, parameter in self.current_parameters_s.items():
                if (
                    parameter == correct_parameter_to_set
                    and str(self.current_values_s[ParamIdx])[0] != "#"
                ):
                    assert parameter in list(self.parameter_to_method_mapping.keys()), (
                        " -  The tag %s is not defined." % parameter
                    )
                    self.parameter_to_method_mapping[parameter][1](
                        self.current_values_s[ParamIdx]
                    )
                    break
        if self.sys_mode == "":
        ...
        if self.benchmark:
            print(
                " -  CxSystem is performing benchmarking. The Brian2 "
    ...
    def _set_runtime(self, *args):
        assert (
            "*" in args[0]
        ), " -  Please specify the unit for the runtime parameter, e.g. um , mm "
    ...
    def _set_sys_mode(self, *args):
        assert args[0] in [
            "local",
            "expanded",
        ], " -  System mode should be either local or expanded. "
        self.sys_mode = args[0]

    def save_input_video(self, *args):
        assert int(eval(args[0])) == 0 or int(eval(args[0])) == 1, (
            " -  The save_input_video flag should be either 0 or 1 but it is %s ."
            % args[0]
        )
    ...
    def _set_grid_radius(self, *args):
        assert (
            "*" in args[0]
        ), " -  Please specify the unit for the grid radius parameter, e.g. um , mm "
        self.general_grid_radius = eval(args[0])
        try:
            if self.scale != 1:
                print(
                    " -  Radius of the system scaled to %s from %s"
                    % (
                        str(np.sqrt(self.scale) * self.general_grid_radius),
                        str(self.general_grid_radius),
                    )
                )
            self.general_grid_radius = np.sqrt(self.scale) * self.general_grid_radius
            if self.sys_mode != "expanded" and self.scale != 1:
                print(
                    " -  System is scaled by factor of %f but the system "
    ...
    def _set_min_distance(self, *args):
        assert (
            "*" in args[0]
        ), " -  Please specify the unit for the minimum distance parameter, e.g. um , mm "
        self.min_distance = eval(args[0])

    def set_import_connections_path(self, *args):
        if (
            len(args) > 0
        ):  # arguments exists if the path is to be set, but the loading won't happen until on of the connections needs it
    ...
    def load_positions(self, *args):
        assert int(eval(args[0])) == 0 or int(eval(args[0])) == 1, (
            " -  The load_positions flag should be either 0 or 1 but it is %s ."
            % args[0]
        )
    ...
    def set_benchmark(self, *args):
        assert int(eval(args[0])) in [
            0,
            1,
        ], " -  Do benchmark flag should be either 0 or 1"
    ...
    def set_profiling(self, *args):
        assert int(eval(args[0])) in [
            0,
            1,
        ], " -  Profiling flag should be either 0 or 1"
    ...
    def neuron_group(self):
        ...
        * ng_init: NeuronGroups() should be initialized with a random vm, ge and gi values. To address this, a 6-line code
        ...
        self.workspace.results["Neuron_Groups_Parameters"][_dyn_neurongroup_name] = (
            self.customized_neurons_list[-1]
        )
        ...
        exec(
            "%s= b2.NeuronGroup(%s, model=%s, method='%s', threshold=%s, reset=%s,refractory = %s, namespace = %s)"
            % (
                _dyn_neurongroup_name,
        ...
        try:
            background_rate = next(
                iter(
                    self.physio_config_df.loc[
                        np.where(self.physio_config_df.values == "background_rate")[0]
                    ]["Value"]
                )
            )
            background_rate_inhibition = next(
                iter(
                    self.physio_config_df.loc[
                        np.where(
                            self.physio_config_df.values == "background_rate_inhibition"
                        )[0]
                    ]["Value"]
                )
            )
        ...
        if (background_rate is not None) or (background_rate_inhibition is not None):
            ...
            if neuron_type in ["L1i", "BC", "MC"]:
                ...
                if flag_bg_calcium_scaling == 1:
                    background_weight = repr(
                        bg_synapse.scale_by_calcium(ca, background_weight)
                    )

                    background_weight_inhibition = repr(
                        bg_synapse_inh.scale_by_calcium(
                            ca, background_weight_inhibition
                        )
                    )
                else:
            ...
            if neuron_type != "PC":
                ...
                for receptor in exc_receptors:
                    ...
                    exec(
                        "%s = b2.PoissonInput(target=%s, target_var='%s_soma', N=%s, rate=%s, weight=%s)"
                        % (
                            poisson_target,
                            _dyn_neurongroup_name,
                            receptor,
                ...
                target_comp_list.extend(
                    ["a" + str(i) for i in range(1, n_target_compartments - 2 + 1)]
                )

                for receptor in exc_receptors:
                    for target_comp in target_comp_list:
                        poisson_target = "bg_%s_%s_%s" % (
                            receptor,
                            _dyn_neurongroup_name,
                            target_comp,
                        )
                        exec(
                            "%s = b2.PoissonInput(target=%s, target_var='%s_%s', N=%s, rate=%s, weight=%s)"
                            % (
                                poisson_target,
                                _dyn_neurongroup_name,
                                receptor,
        ...
        try:
            exec(
                "%s.x=b2.real(self.customized_neurons_list[%d]['w_positions'])*mm\n"
        ...
        if self.init_vms:
            ...
            ng_init += (
                "\t\tsetattr(%s,_key,%s['V_init_min']+Vr_offset * (%s['V_init_max']-%s['V_init_min']))\n"
                % (
                    _dyn_neurongroup_name,
                    _dyn_neuron_namespace_name,
                    _dyn_neuron_namespace_name,
    ...
    def synapse(self):
        ...
        * syn_con_str: The string containing the syntax for connect() method of a current Synapses() object. This string
        ...
        assert not np.any(
            self.current_values_s.loc[obligatory_indices].isnull()
        ), ' -  Following obligatory values cannot be "--":\n%s' % str(
            [_all_columns[ii] for ii in _obligatory_params]
        )
        assert len(self.current_values_s) == self.current_parameters_s_orig_len, (
            ...
            % str(
                list(
                    self.anat_and_sys_conf_df.loc[self.value_line_idx]
                    .to_dict()
                    .values()
                )
            )
        ...
        index_of_post_syn_idx = int(
            np.where(self.current_parameters_s.values == "post_syn_idx")[0][0]
        )
        index_of_pre_syn_idx = int(
            np.where(self.current_parameters_s.values == "pre_syn_idx")[0][0]
        )
        index_of_syn_type = int(
            np.where(self.current_parameters_s.values == "syn_type")[0][0]
        )
        try:
        ...
        try:
            ...
            pass
        try:
            index_of_monitors = int(
                np.where(self.current_parameters_s.values == "monitors")[0][0]
            )
        except TypeError:
            pass
        try:
        ...
        if len(current_post_syn_idx) > 1 and "[" in current_post_syn_idx:
            ...
            current_post_syn_tags = current_post_syn_idx[
                current_post_syn_idx.index("[") : current_post_syn_idx.index("]") + 1
            ]
            ...
            if current_post_syn_tags == "[C]":  # [C] means the target is a compartment
                _post_group_idx, _post_com_idx = current_post_syn_idx.split(
                    "[" + "C" + "]"
                )
                ...
                pre_group_ref_idx = [
                    self.customized_neurons_list.index(tmp_group)
                    for tmp_group in self.customized_neurons_list
                    if tmp_group["idx"] == int(current_pre_syn_idx)
                ][0]
                post_group_ref_idx = [
                    self.customized_neurons_list.index(tmp_group)
                    for tmp_group in self.customized_neurons_list
                    if tmp_group["idx"] == int(_post_group_idx)
                ][0]
                assert (
                ...
                self.current_parameters_s = pd.concat(
                    [
                        self.current_parameters_s,
                        pd.Series(["pre_type", "post_type", "post_comp_name"]),
                    ],
                    ignore_index=True,
                )
                self.current_values_s = pd.concat(
                    [self.current_values_s, pd.Series([_pre_type, _post_type])],
                    ignore_index=True,
                )
                ...
                if str(_post_com_idx)[0] == "0":
                    assert len(_post_com_idx) > 1, (
                        " -  The soma layer of a compartmental neuron is being targeted, but the exact "
                    ...
                    for idx, tmp_idx in enumerate(_post_com_idx[1:]):
                        ...
                        if np.any(self.current_parameters_s.str.contains("p")):
                            tmp_args[index_of_p] = (
                                _current_probs[idx]
                                if str(tmp_args[index_of_p]) != "--"
                                else "--"
                            )
                        if np.any(self.current_parameters_s.str.contains("n")):
                            tmp_args[index_of_n] = (
                                _current_ns[idx]
                                if str(tmp_args[index_of_n]) != "--"
                                else "--"
                            )
                        if tmp_idx.lower() == "b":
            ...
            pre_group_ref_idx = [
                self.customized_neurons_list.index(tmp_group)
                for tmp_group in self.customized_neurons_list
                if int(tmp_group["idx"]) == int(current_pre_syn_idx)
            ][0]
            post_group_ref_idx = [
                self.customized_neurons_list.index(tmp_group)
                for tmp_group in self.customized_neurons_list
            ...
            self.current_values_s = pd.concat(
                [self.current_values_s, pd.Series([_pre_type, _post_type, "_soma"])],
                ignore_index=True,
            )
            self.current_parameters_s = pd.concat(
                [
        ...
        for syn in self.current_values_s:
            ...
            pre_type_idx = next(
                iter(
                    self.current_parameters_s[
                        self.current_parameters_s == "pre_type"
                    ].index
                )
            )
            ...
            post_type_idx = next(
                iter(
                    self.current_parameters_s[
                        self.current_parameters_s == "post_type"
                    ].index
                )
            )
            ...
            post_comp_idx = next(
                iter(
                    self.current_parameters_s[
                        self.current_parameters_s == "post_comp_name"
                    ].index
                )
            )
            ...
            try:
                custom_weight_idx = next(
                    iter(
                        self.current_parameters_s[
                            self.current_parameters_s == "custom_weight"
                        ].index
                    )
                )
            ...
            try:
                multiply_weight_idx = self.current_parameters_s[
                    self.current_parameters_s == "multiply_weight"
                ].index.values[0]
            ...
            self.customized_synapses_list.append(
                SynapseReference(
                    receptor,
                    pre_syn_idx,
                    post_syn_idx,
            ...
            _pre_group_idx = self.neurongroups_list[
                self.customized_synapses_list[-1]["pre_group_idx"]
            ]
            _post_group_idx = self.neurongroups_list[
            ...
            try:
                index_of_load_connection = int(
                    np.where(self.current_parameters_s.values == "load_connection")[0][
                        0
                    ]
                )
                if "-->" in syn[index_of_load_connection]:
                    self.default_load_flag = int(
                        syn[index_of_load_connection].replace("-->", "")
                    )
                    ...
                    load_connection_idx = next(
                        iter(
                            self.current_parameters_s[
                                self.current_parameters_s == "load_connection"
                            ].index
                        ),
                        "no match",
                    )
            ...
            if (
                self.default_load_flag == 1
                or (self.default_load_flag == -1 and _do_load == 1)
            ) and not hasattr(self, "imported_connections"):
                # Only load the file if it's not loaded already, and if the connections are supposed to be loaded from the file
                self.set_import_connections_path()

            try:
                index_of_save_connection = int(
                    np.where(self.current_parameters_s.values == "save_connection")[0][
                        0
                    ]
                )
                if "-->" in syn[index_of_save_connection]:
                    self.default_save_flag = int(
                        syn[index_of_save_connection].replace("-->", "")
                    )
            ...
            if (
                self.default_load_flag == 1
                or (self.default_load_flag == -1 and _do_load == 1)
            ) and hasattr(self, "imported_connections"):
                assert _syn_ref_name in list(self.imported_connections.keys()), (
                    " -  The data for the following connection was not found in the loaded brian data: %s"
                    % _syn_ref_name
                )
                ...
                try:
                    ...
                    # Load weights
                    eval(_dyn_syn_name).wght = (
                        b2.repeat(
                            self.imported_connections[_syn_ref_name]["data"].data
                    ...
                    if "delay" in self.imported_connections[_syn_ref_name].keys():
                        eval(_dyn_syn_name).delay = (
                            b2.repeat(
                                self.imported_connections[_syn_ref_name]["delay"].data
                                / int(self.imported_connections[_syn_ref_name]["n"]),
                                int(self.imported_connections[_syn_ref_name]["n"]),
                            )
                            * second
                ...
                print(
                    " -  Synaptic connection is set to be loaded, however "
            ...
            if self.default_load_flag == 0 or (
                self.default_load_flag == -1 and _do_load == 0
            ):
            ...
            if self.device == "python":
                ...
                try:
                    n_idx = next(
                        iter(
                            self.current_parameters_s[
                                self.current_parameters_s == "n"
                            ].index
                        ),
                        "no match",
                    )
                    _current_connections = int(num_tmp / float(syn[n_idx])) / len(
                        self.current_values_s
                    )
                except ValueError:
                    print(
                        " -  number of synapses for last connection was "
                ...
                try:
                    print(
                        " -  %s%s to %s" % (_load_str, _pre_group_idx, _post_group_idx)
                    )
        ...
        * number_of_neurons: The number of neurons that exist in the input .mat file.
        ...
        if not hasattr(self.workspace, "simulation_name"):
            raise ParameterNotFoundError(
                "The parameter 'simulation_title' is not defined in the configuration file "
            )

        def video(self):
            print(" -  Creating an input based on the video input ...")
            input_mat_path = self.current_values_s[
                self.current_parameters_s[
                    self.current_parameters_s == "path"
                ].index.item()
            ]
            freq = self.current_values_s[
                self.current_parameters_s[
                    self.current_parameters_s == "freq"
                ].index.item()
            ]
            inp = Stimuli(
                duration=self.runtime,
                input_mat_path=input_mat_path,
                output_folder=self.workspace.get_simulation_folder_as_posix(),
                output_file_suffix=self.suffix,
                output_file_extension=self.workspace.get_output_extension(),
            )
            ...
            def waitress(self):
                ...
                while proc.is_alive():
                    time.sleep(1)
                spk_generator_sp, spk_generator_ti, thread_number_of_neurons = (
                    inp.load_input_seq(self.workspace.get_simulation_folder_as_posix())
                )
                if not self.save_input_video:
                    ...
                    os.remove(
                        ...
                        .joinpath(
                            "input"
                            + self.suffix
                            + self.workspace.get_output_extension()
                        )
                        .as_posix()
                    )
                ...
                thread_nt_name = self._NeuronThreshold_prefix + str(
            ...
            save_conn_idx = synapse_def_line[
                synapse_def_line == "save_connection"
            ].index[0]
            for conn_load_item in input_synaptic_lines[load_conn_idx]:
                if "<--" in conn_load_item or "-->" in conn_load_item:
                    for synaptic_row in range(
                        input_synaptic_lines.index[0] + 1,
                        self.anat_and_sys_conf_df[0].index[-1],
                    ):
                        if self.anat_and_sys_conf_df.loc[synaptic_row][0][0] != "#":
                            sign_to_add = "-->" if "-->" in conn_load_item else "<--"
                            self.anat_and_sys_conf_df.loc[
                                input_synaptic_lines.index[0] + 1, load_conn_idx
                            ] += sign_to_add
                            break
                ...
                if "<--" in conn_save_item or "-->" in conn_save_item:
                    for synaptic_row in range(
                        ...
                        if self.anat_and_sys_conf_df.loc[synaptic_row][0][0] != "#":
                            ...
                            self.anat_and_sys_conf_df.loc[
                                input_synaptic_lines.index[0] + 1, save_conn_idx
                            ] += sign_to_add
                            break
            self.awaited_conf_lines = (
                synapse_def_line.to_frame()
                .transpose()
                .reset_index(drop=True)
                .append(input_synaptic_lines)
                .reset_index(drop=True)
            )
            self.anat_and_sys_conf_df = self.anat_and_sys_conf_df.drop(
                input_synaptic_lines.index.tolist()
            ).reset_index(drop=True)
        ...
        def VPM(self):  # ventral posteromedial (VPM) thalamic nucleus
            spike_times_idx = next(
                iter(
                    self.current_parameters_s[
                        self.current_parameters_s == "spike_times"
                    ].index
                )
            )
            ...
            num_of_neurons_idx = next(
                iter(
                    self.current_parameters_s[
                        self.current_parameters_s == "number_of_neurons"
                    ].index
                )
            )
            ...
            if "[act]" in spike_times:
                spike_times_unit = spike_times[
                    spike_times.index("*") + 1 : spike_times.index("[act]")
                ]
                active_neurons_str = "arange" + spike_times[
                    spike_times.index("[act]") + 5 :
                ].replace("-", ",")
            else:
            ...
            try:
                tmp_net_center_idx = next(
                    iter(
                        self.current_parameters_s[
                            self.current_parameters_s == "net_center"
                        ].index
                    ),
                    "no match",
                )
            ...
            radius_idx = next(
                iter(
                    self.current_parameters_s[
                        self.current_parameters_s == "radius"
                    ].index
                )
            )
            ...
            if spike_times_unit == "Hz":
                assert (
                    b2.asarray(spike_times_list).size == 1
            ...
            if hasattr(self, "imported_connections"):  # load the positions if available
                ...
                self.customized_neurons_list[current_idx]["z_positions"] = (
                    vpm_customized_neuron.output_neuron["z_positions"]
                )
                self.customized_neurons_list[current_idx]["w_positions"] = (
        ...
        def spikes(self):
            ...
            ng_name = (
                self._NeuronGroup_prefix
                + str(self.spike_input_group_idx)
                + "_relay_spikes"
            )
        ...
        ), " -  The type of the input is not defined in the configuration file."
        ...
        assert len(self.current_parameters_s) <= len(
            input_type_to_method_mapping[_input_type][0]
        ), (
            " -  Too many parameters for the\
         current %s input. The parameters should be consist of:\n %s"
            % (_input_type, input_type_to_method_mapping[_input_type][0])
        )
        obligatory_columns = list(
            np.array(input_type_to_method_mapping[_input_type][0])[
        ...
        try:
            ...
            mons = self.current_values_s[
                next(
                    iter(
                        self.current_parameters_s[
                            self.current_parameters_s == "monitors"
                        ].index
                    ),
                    "no match",
                )
        ...
        group_idx = self.current_values_s[
            next(
                iter(
                    self.current_parameters_s[self.current_parameters_s == "idx"].index
                )
            )
        ]

        assert (
            group_idx not in self.NG_indices
    ...
    def gather_result(self):
        ...
        if self.do_save_connections:
            ...
            self.workspace.connections["positions_all"]["z_coord"] = (
                self.workspace.results["positions_all"]["z_coord"]
            )
...
if __name__ == "__main__":
    root = Path(os.getcwd())
    os.chdir(root)
    anatomy_path = root.joinpath(
        "tests", "config_files", "pytest_Anatomy_config.csv"
    ).as_posix()
    physio_path = root.joinpath(
