...
class ArrayRun:
    ...
    def __init__(
        ...
        self.param_search_num_of_params = len(self.anatomy_arrun_cell_indices) + len(
    ...
    def _prepare_multi_dim_arrun_metadata(self):
        meta_columns = []
        for meta_col_idx in range(
            1,
            1
            + len(self.anatomy_arrun_cell_indices)
            + len(self.physio_arrun_cell_indices),
        ):
        ...
        if self.anatomy_arrun_cell_indices:
            anat_variations, anatomy_namings = (
                self.generate_dataframes_for_param_search(
                    self.anatomy_df, self.anatomy_arrun_cell_indices, df_type="anatomy"
                )
            )
        if self.physio_arrun_cell_indices:
            physio_variations, physio_namings = (
                self.generate_dataframes_for_param_search(
                    self.physiology_df,
                    self.physio_arrun_cell_indices,
                    df_type="physiology",
        ...
        if self.anatomy_arrun_cell_indices and self.physio_arrun_cell_indices:
            for anat_idx, anat_df in enumerate(anat_variations):
                for physio_idx, physio_df in enumerate(physio_variations):
                    ...
                    physio_without_timeline = "_" + "_".join(
                        physio_namings[physio_idx].split("_")[3:]
                    )
                    self.final_namings.append(
                        anatomy_namings[anat_idx] + physio_without_timeline
                    )
        ...
        product_of_parameters = list(
            itertools.product(*list(self.metadata_dict.values()))
        )
        self.tmp_df = pd.DataFrame(
            product_of_parameters, columns=list(self.metadata_dict.keys())
        )
        ...
        for col_idx, col_title in enumerate(self.all_titles):
            ...
            self.final_metadata_df["Dimension-%d Value" % (col_idx + 1)] = self.tmp_df[
                col_title
            ]
    ...
    def _prepare_one_dim_arrun_metadata(self):
        ...
        if self.anatomy_arrun_cell_indices:
            ...
            self.list_of_physio_dfs.extend(
                [self.default_physio_df for _ in range(len(self.list_of_anatomy_dfs))]
            )
            self.final_namings.extend(anatomy_namings)
        if self.physio_arrun_cell_indices:
            df_phys_array, physio_namings = self.generate_dataframes_for_param_search(
                self.physiology_df, self.physio_arrun_cell_indices, df_type="physiology"
            )
            self.list_of_physio_dfs.extend(df_phys_array)
            self.list_of_anatomy_dfs.extend(
                [self.default_anatomy_df for _ in range(len(self.list_of_physio_dfs))]
            )
        ...
        index_len = len(
            [item for sublist in list(self.metadata_dict.values()) for item in sublist]
        )
    ...
    def _check_single_experiment_multi_trials(self):
        if not self.list_of_anatomy_dfs and not self.list_of_physio_dfs:
            print(
                " -  No parameter for array run found, the default configurations are going to be simulated %d times"
                % self.trials_per_config
            )
            self.list_of_anatomy_dfs = [
                self.default_anatomy_df
            ] * self.trials_per_config
    ...
    def generate_dataframes_for_param_search(
        ...
        array_variable = original_df.iloc[
            index_of_array_variable[0][0], index_of_array_variable[0][1]
        ]

        opening_braket_idx = array_variable.index("{")
        if (
            not self.multidimension_array_run and self.param_search_num_of_params > 1
        ) or (self.param_search_num_of_params == 1 and ":" in array_variable):
            colon_idx = array_variable.index(":")
            array_variable = array_variable.replace(
                array_variable[opening_braket_idx + 1 : colon_idx + 1], ""
            )  # removing default value
        elif ":" in array_variable:
            print(
                " -  The default value set for %s is omitted since the "
        ...
        template_of_variable = (
            array_variable[:opening_braket_idx]
            + "^^^"
            + array_variable[closing_braket_idx + 1 :]
        )
        assert not ("|" in array_variable and "&" in array_variable), (
            " -  The following array run should be defined either using | or & not both of them:"
            % array_variable
        )
        variables_to_iterate = []
        if "|" in array_variable:
            changing_part = array_variable[
                opening_braket_idx + 1 : closing_braket_idx
            ].replace("|", ",")
            decimal_type_list = [decimal.Decimal(vv) for vv in changing_part.split(",")]
            variables_to_iterate = np.arange(
                decimal_type_list[0], decimal_type_list[1], decimal_type_list[2]
            ).tolist()
        elif "&" in array_variable:
            variables_to_iterate = eval(
                '["'
                + array_variable[opening_braket_idx + 1 : closing_braket_idx].replace(
                    "&", '","'
                )
                + '"]'
            )
        ...
        for var_idx, var in enumerate(
            ...
            if self.multidimension_array_run and len(index_of_array_variable) > 1:
                tmp_title, tmp_value, tmp_naming = self.filename_generator(
                    temp_df, index_of_array_variable, df_type
                )
                temp_df, namings = self.generate_dataframes_for_param_search(
                    temp_df,
                    index_of_array_variable[1:],
                    df_type,
                    tmp_naming.replace(self.suffix, ""),
                    recursion_counter=recursion_counter + 1,
                )
            else:
                temp_df = [self.default_df_extractor(temp_df)]
                tmp_title, tmp_value, tmp_naming = self.filename_generator(
                    temp_df[0], index_of_array_variable, df_type
                )
                namings = [
                    self.suffix + naming_prefix + tmp_naming.replace(self.suffix, "")
                ]
        ...
        if not self.multidimension_array_run and len(index_of_array_variable) > 1:
            temp_df, namings = self.generate_dataframes_for_param_search(
                original_df, index_of_array_variable[1:], df_type, naming_prefix=""
            )
            array_of_dfs.extend(temp_df)
            run_namings.extend(namings)
    ...
    def default_df_extractor(self, df_):
        ...
        arrays_idx_ = [
            (df_search_result[0][i], df_search_result[1][i])
            for i in range(len(df_search_result[0]))
        ]
        for to_default_idx in arrays_idx_:
            value_to_default = df.loc[to_default_idx[0]][to_default_idx[1]]
            assert ":" in value_to_default, (
                " -  The default value should be defined for %s if the system is running for one-dimensional parameter search"
                % value_to_default
            )
            default = value_to_default[
                value_to_default.index("{") + 1 : value_to_default.index(":")
            ]
            df.loc[to_default_idx[0]][to_default_idx[1]] = df.loc[to_default_idx[0]][
                to_default_idx[1]
            ].replace(
                value_to_default[
                    value_to_default.index("{") : value_to_default.index("}") + 1
                ],
                default,
            )
        return df
    ...
    def filename_generator(self, df, idx, df_type):
        ...
        idx = idx[0]
        whitelist = set(
            "abcdefghijklmnopqrstuvwxyABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890_-"
        )

        if np.any(df.iloc[:, 0].str.contains("row_type")):  # Anatomical conf
            definition_rows_indices = np.array(
                df[0][df[0] == "row_type"].index.tolist()
            )
            target_row_ix = max(
                np.where(definition_rows_indices < idx[0])[0]
            )  # get the index of the row containing the column info
        ...
        naming = (
            self.suffix + "_" + title + "".join(filter(whitelist.__contains__, value))
        )
    ...
    def _get_multidim_array_run_flag(self):
        ...
        try:
            multi_dim_flag = int(
                eval(parameter_finder(self.anatomy_df, "multidimension_array_run"))
            )
        except TypeError:
            pass
    ...
    def _get_trials_per_config(self):
        ...
        try:
            ...
            print(
                " - trials_per_config is not defined in the configuration "
