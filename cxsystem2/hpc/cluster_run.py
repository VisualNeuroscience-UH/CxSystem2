...
class ClusterRun:
    ...
    def __init__(self, array_run_obj, anat_file_path, physio_file_path, suffix=""):
        ...
        try:
            self.cluster_workspace = PurePosixPath(
                parameter_finder(array_run_obj.anatomy_df, "cluster_workspace")
            )
        except NameError:
        ...
        assert (
            self.cluster_workspace.is_absolute()
        ), "cluster_workspace {} must be an absolute path with explicit [remote] home directory path".format(
            self.cluster_workspace.as_posix()
        )

        try:
            self.cluster_repospace = PurePosixPath(
                parameter_finder(array_run_obj.anatomy_df, "remote_repo_path")
            )
            assert (
                self.cluster_repospace.is_absolute()
            ), "remote_repo_path {} must be an absolute path with explicit [remote] home directory path".format(
                self.remote_repo_path.as_posix()
            )
        except NameError:
            print(
                " -  Remote_repo_path not found, assuming remote_repo_path = [cluster_workspace]/CxSystem2"
            )  # inside method update_remote_cxsystem2
            self.cluster_repospace = None

        try:
            self.cluster_address = parameter_finder(
                array_run_obj.anatomy_df, "cluster_address"
            )
        except NameError:
            raise ParameterNotFoundError(
        ...
        try:
            ...
            print(
                " -  No cluster login node found. Directly conencting to cluster address {}".format(
        ...
        try:
            self.cluster_username = parameter_finder(
                array_run_obj.anatomy_df, "cluster_username"
            )
            assert (
        ...
        try:
            ...
            if "CLUSTERPASS" in os.environ.keys():
                ...
                self.password = getpass.getpass(
                    " -  Enter password for user {}: " "".format(self.cluster_username)
                )
        ...
        if self.cluster_login_node != "--":
            ...
            sock = paramiko.ProxyCommand(
                "ssh {}@{} nc {} 22".format(
                    self.cluster_username, self.cluster_login_node, self.cluster_address
                )
            )
            sock.settimeout(30)
            self.client.connect(
        ...
        try:
            imported_connections_file = Path(
                parameter_finder(array_run_obj.anatomy_df, "import_connections_from")
            )
            if imported_connections_file.is_file():
                scp.put(
                    imported_connections_file.as_posix(),
                    self.cluster_workspace.as_posix(),
                )
                new_path = (
        ...
        scp.put(
            anat_file_path.as_posix(),
            self.cluster_workspace.joinpath(self.remote_anat_filename).as_posix(),
        )
        scp.put(
            physio_file_path.as_posix(),
            self.cluster_workspace.joinpath(self.remote_phys_filename).as_posix(),
        )
        ...
        self.slurm_file_path = Path(
            parameter_finder(array_run_obj.anatomy_df, "cluster_job_file_path")
        ).expanduser()
        ...
        self.update_remote_cxsystem2(
            self.slurm_file_path, self.cluster_workspace, self.cluster_repospace
        )
        ...
        for item_idx, item in enumerate(array_run_obj.clipping_indices):
            ...
            scp.put(
                self.local_cluster_folder.joinpath(remote_slurm_filename).as_posix(),
        ...
        cluster_metadata = {
            ...
            "local_cluster_run_download_folder": self.local_cluster_folder.joinpath(
                "downloads"
            ).as_posix(),
            "cluster_workspace": self.cluster_workspace.as_posix(),
            "cluster_simulation_folder": self.cluster_workspace.joinpath(
                parameter_finder(array_run_obj.anatomy_df, "simulation_title")
            ).as_posix(),
            "suffix": self.suffix,
        ...
        with open(
            self.local_cluster_folder.joinpath(
                "cluster_metadata{}.pkl".format(self.suffix)
            ),
            "wb",
        ) as ff:
        ...
        metadata_path = self.local_cluster_folder.joinpath(
            "cluster_metadata{}.pkl".format(self.suffix)
        )
    ...
    def update_remote_cxsystem2(
        self, slurm_path, remote_workspace, remote_repospace=None
    ):
        slurm_path = Path(slurm_path)
        remote_workspace = Path(remote_workspace)
        module_name, venv_activation = self.find_remote_python_module(
            slurm_path
        )  # This examines slurm job file
        self.ssh_commander("mkdir -p {}".format(self.cluster_workspace.as_posix()))
        ...
        cxsystem_version_rawstring = self.ssh_commander(
            f"source ~/.bashrc ; {venv_activation}; cxsystem2 -v"
        )
        ...
        if version.parse(cxsystem_version) >= version.parse(
            first_cxsystem_version_for_virtual_env
        ):
            git_repo_error_message = ""
            print(f" -  Found CxSystem2 version {cxsystem_version} installed")
            installed_repo_init_module = self.ssh_commander(
                'source ~/.bashrc ; python -c "import cxsystem2; print(cxsystem2.__file__)"'
            ).decode("utf-8")
            if str(remote_repospace.as_posix()) not in installed_repo_init_module:
                print(
                    f" -  NOTE {str(remote_repospace)} not in {installed_repo_init_module}, best to re-install"
                )
            ...
            git_repo_error_message = self.ssh_commander(
                "source ~/.bash_profile ; "
                "source ~/.bashrc ; "
                "cd {repospace} ; "
                "git -C . rev-parse".format(repospace=remote_repospace.as_posix())
            ).decode("utf-8")
        if not git_repo_error_message:
            git_basename = self.ssh_commander(
                "cd {repospace} ; "
                "git rev-parse --show-toplevel".format(
                    repospace=remote_repospace.as_posix()
                )
            ).decode("utf-8")
            commit_HEAD_hash = self.ssh_commander(
                "cd {repospace} ; "
                "git rev-parse --short HEAD".format(
                    repospace=remote_repospace.as_posix()
                )
            ).decode("utf-8")
            git_branch = self.ssh_commander(
                "cd {repospace} ; "
                "git rev-parse --abbrev-ref HEAD".format(
                    repospace=remote_repospace.as_posix()
                )
            ).decode("utf-8")
            print(
            ...
            print(
                self.ssh_commander(
                    ...
                    "cd CxSystem2 ; "
                    "git pull ; ".format(workspace=remote_workspace.as_posix())
                ).decode("utf-8")
            )
            if module_name:
                print(
                    self.ssh_commander(
                        ...
                        "module load {module} ;"
                        "cd {cxfolder} ; "
                        "python -m pip install -Ue . --user'".format(
                            module=module_name,
                            cxfolder=remote_workspace.joinpath("CxSystem2").as_posix(),
                        )
                    ).decode("utf-8")
                )
                ...
                print(
                    self.ssh_commander(
                        ...
                        "cd {cxfolder} ; "
                        "python -m pip install -Ue . --user'".format(
                            venv=venv_activation,
                            cxfolder=remote_workspace.joinpath("CxSystem2").as_posix(),
                        )
                    ).decode("utf-8")
                )

    def find_remote_python_module(self, slurm_path):
        module_name = ""
        venv_activation = ""
        slurm_path = Path(slurm_path)
        with open(slurm_path.as_posix()) as f:
            for line in f:
                ...
                elif "conda activate" in line.lower() or (
                    "source" in line.lower() and "activate" in line.lower()
                ):
                    venv_activation = line.strip("\n")

        if module_name and venv_activation:
            print(
                "NOTE: You have both virtual environment activation and module load strings in your slurm job file, setting module priority"
            )
            venv_activation = ""
        ...
        if venv_activation:
            print(
                " -  Remote virtual environment is activated with command {}".format(
                    venv_activation
                )
            )
...
class ClusterDownloader:
    def __init__(self, metapath, clean_remote=False, retrieve=True):
        ...
        if not self.metadata_path.is_file():
            raise FileNotFoundError(
                "Cluster run metadata file not found: {}".format(
                    self.metadata_path.as_posix()
                )
            )
        self.metadata = self.load_metadata()
        submission_time = datetime.strptime(
            self.metadata["timestamp"], "%Y-%m-%d %H:%M:%S"
        )
        ...
        if "CLUSTERPASS" in os.environ.keys():
            ...
            self.password = getpass.getpass(
                "Enter password for user {}: ".format(self.metadata["cluster_username"])
            )
        if self.metadata["cluster_login_node"] != "--":
            print(
                " -  Connecting to login node {}".format(
                    self.metadata["cluster_login_node"]
                )
            )
            sock = paramiko.ProxyCommand(
                "ssh {}@{} nc {} 22".format(
                    self.metadata["cluster_username"],
                    self.metadata["cluster_login_node"],
    ...
    def retrieve(self):
        ...
        if type(self.metadata["local_cluster_run_download_folder"]) is str:
            self.metadata["local_cluster_run_download_folder"] = Path(
                self.metadata["local_cluster_run_download_folder"]
            )
        ...
        while waiting_flag:
            ...
            # just a better check:
            time.sleep(2)
            check_remote = self.ssh_commander(
                self.client,
            ...
            if not self.metadata["cluster_username"] in check_remote:
                ...
                for item in (
                    self.ssh_commander(
                        self.client,
                        "cd {}; ls".format(self.metadata["cluster_simulation_folder"]),
                        0,
                    )
                    .decode("utf-8")
                    .split("\n")
                    ...
                    if item != "" and self.metadata["suffix"] in item:
                        self.scpclient.get(
                            Path(self.metadata["cluster_simulation_folder"])
                            .joinpath(item)
                            .as_posix(),
                            Path(self.metadata["local_cluster_run_download_folder"])
                            .joinpath(item)
                            .as_posix(),
                        )
                        print("\t", item)
        ...
        print(
            " -  Downloads are available in: {}".format(
                self.metadata["local_cluster_run_download_folder"]
            )
        )
    ...
    def clean_remote(self):
        self.ssh_commander(
            self.client, "rm -rf {}".format(self.metadata["cluster_workspace"]), 0
        ).decode("utf-8").split("\n")
        self.ssh_commander(
            self.client,
            "rm -rf {}".format(self.metadata["local_workspace_unexpanded"]),
            0,
        ).decode("utf-8").split("\n")
...
if __name__ == "__main__":
    cl = ClusterDownloader(
        "/home/corriel/CxPytestWorkspace/cluster_run_20191127_1223593/cluster_metadata_20191127_1223593.pkl",
        retrieve=False,
    )
    cl.clean_remote()
