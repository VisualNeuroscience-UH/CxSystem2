...
class ConfigConverter:
        ...
        if self.original_path.suffix == ".json":
            ...
            if not self.overwrite:
                ...
                while self.new_filepath.is_file():
                    new_filename = (
                        self.filename_without_extension + "_" + str(counter) + ".csv"
                    )
                    self.new_filepath = self.directory.joinpath(new_filename)
                    counter += 1
            ...
            with open(self.new_filepath.as_posix(), "w", newline="") as f:
                ...
                if "params" in json_data.keys():
                    ...
                    csv_file.writerow(
                        ["row_type"]
                        + list(json_data["params"].keys())
                        + [""] * (total_number_of_rows - column_num)
                    )
                    csv_file.writerow(
                        ["params"]
                        + list(json_data["params"].values())
                        + [""] * (total_number_of_rows - column_num)
                    )
                ...
                if "S" in json_data.keys():
                    ...
                    for entry in json_data["physio_data"]:
                        if (
                            "Key-Value" in entry.keys()
                        ):  # this means it's a key-value entry
    ...
    def json_data_to_csv(self):
        ...
        if "params" in self.json_data.keys():
            ...
            tmp_keys_in_string_format = [
                str(x) for x in self.json_data["params"].keys()
            ]
            csv_string += (
                ",".join(
                    ["row_type"]
            ...
            tmp_values_in_string_format = [
                str(x) for x in self.json_data["params"].values()
            ]
            csv_string += (
                ",".join(
                    ["params"]
                ...
                if "Key-Value" in entry.keys():  # this means it's a key-value entry
                    for key_val_idx, key_value in enumerate(entry["Key-Value"]):
                        if key_val_idx == 0:
                            row = [
                                str(entry["Variable"]),
                                str(key_value["Key"]).replace(",", "{comma}"),
                                str(key_value["Value"]).replace(",", "{comma}"),
                    ...
                    row = [
                        str(entry["Variable"]).replace(",", "{comma}"),
                        "",
                        str(entry["Value"]).replace(",", "{comma}"),
    ...
    def csv_to_json(self):
        if self.original_path.suffix == ".csv":
            ...
            if self.overwrite:
                ...
                while self.new_filepath.is_file():
                    new_filename = (
                        self.filename_without_extension + "_" + str(counter) + ".json"
                    )
                    self.new_filepath = self.directory.joinpath(new_filename)
                    counter += 1
            ...
            row_type_rows = list(
                csv_data.loc[csv_data[0] == "row_type"].index.values.astype(int)
            )
            if (
                len(row_type_rows) > 3
            ):  # let's assume a working anatomy config file has at least 3 instance of row_type for params, G and S
                for row_idx in row_type_rows:
                    next_non_comment_line_counter = 1
                    while (
                        "#" in csv_data.loc[row_idx + next_non_comment_line_counter][0]
                    ):
                        next_non_comment_line_counter += 1
                    if (
                        csv_data.loc[row_idx + next_non_comment_line_counter][0]
                        == "params"
                    ):
                        for param_idx, param in enumerate(csv_data.loc[row_idx]):
                            if param != "row_type" and not pd.isna(param):
                                ...
                                try:
                                    self.params[param.strip()] = int(
                                        csv_data.loc[row_idx + 1][param_idx].strip()
                                    )
                                except ValueError:
                                    self.params[param.strip()] = csv_data.loc[
                                        row_idx + 1
                                    ][param_idx].strip()
                    elif (
                        csv_data.loc[row_idx + next_non_comment_line_counter][0] == "IN"
                    ):
                        counter = 0
                        # noinspection DuplicatedCode
                        while csv_data.loc[
                            row_idx + next_non_comment_line_counter + counter
                        ][0] == "IN" or "#" in str(
                            csv_data.loc[
                                row_idx + next_non_comment_line_counter + counter
                            ][0]
                            ...
                            ):  # pass the commented row
                                tmp_input = {}
                                for param_idx, param in enumerate(
                                    csv_data.loc[row_idx]
                                ):
                                    if param != "row_type" and not pd.isna(param):
                                        try:
                                            tmp_input[param.strip()] = int(
                                                csv_data.loc[
                                                    row_idx
                                                    + next_non_comment_line_counter
                                                    + counter
                                                ][param_idx].strip()
                                            )
                                        except ValueError:
                                            tmp_input[param.strip()] = csv_data.loc[
                                                row_idx
                                                + next_non_comment_line_counter
                                                + counter
                                            ][param_idx].strip()
                            ...
                            if (
                                row_idx + next_non_comment_line_counter + counter
                                > csv_data.index.to_list()[-1]
                            ):
                                break

                    elif (
                        csv_data.loc[row_idx + next_non_comment_line_counter][0] == "G"
                    ):
                        counter = 0
                        # noinspection DuplicatedCode
                        while csv_data.loc[
                                ...
                                for param_idx, param in enumerate(
                                    ...
                                    if param != "row_type" and not pd.isna(param):
                                        try:
                                            ...
                                            tmp_group[param.strip()] = csv_data.loc[
                                                row_idx
                                                + next_non_comment_line_counter
                                                + counter
                                            ][param_idx].strip()
                        ...
                        csv_data.loc[row_idx + next_non_comment_line_counter][0] == "S"
                    ):
                        counter = 0
                        # noinspection DuplicatedCode
                        while csv_data.loc[
                                ...
                                for param_idx, param in enumerate(
                                    ...
                                    if param != "row_type" and not pd.isna(param):
                                        try:
                                            ...
                                            tmp_synapse[param.strip()] = csv_data.loc[
                                                row_idx
                                                + next_non_comment_line_counter
                                                + counter
                                            ][param_idx].strip()
                ...
                for var_idx, var in enumerate(csv_data[csv_data.columns[0]]):
                    if (
                        not pd.isna(var)
                        and not var.startswith("#")
                        and var != "Variable"
                    ):
                        key = csv_data[csv_data.columns[1]][var_idx]
                        value = (
                            csv_data[csv_data.columns[2]][var_idx]
                            if not pd.isna(csv_data[csv_data.columns[2]][var_idx])
                            else ""
                        )
                        comment = (
                            csv_data[csv_data.columns[3]][var_idx]
                            if not pd.isna(csv_data[csv_data.columns[3]][var_idx])
                            else ""
                        )
                        if pd.isna(
                            key
                        ):  # this checks if the key is nan, which means it's a key/value pair
                            # var_val = {str(json_idx_counter): {"Variable" : var, "Value":value, "Comment":comment } }
                            # self.output.update(var_val)
                            try:
                                self.output["physio_data"].append(
                                    {
                            ...
                            while True:
                                ...
                                comment = (
                                    csv_data[csv_data.columns[3]][var_idx + counter]
                                    if not pd.isna(
                                        csv_data[csv_data.columns[3]][var_idx + counter]
                                    )
                                    else ""
                                ...
                                if not (
                                    pd.isna(key) and pd.isna(value) and comment == ""
                                ):
                                    try:
                                        key_val = {
                                            "Key": str(key).strip(" "),
                                ...
                                if (
                                    var_idx + counter + 1 > csv_data.index.to_list()[-1]
                                ) or not pd.isna(
                                    csv_data[csv_data.columns[0]][var_idx + counter + 1]
                                ):
                                    break

                            self.output["physio_data"].append(
                                {"Variable": str(var).strip(" "), "Key-Value": tmp_var}
                            )
                        json_idx_counter += 1
