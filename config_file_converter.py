...
class filetype_converter():
    def __init__(self,path):
        self.path = path
        self.directory = self.path.parent.absolute()
        self.filename_without_extension = self.path.stem
        self.output = {}
        self.params = {}
        self.IN = {}
        self.G = {}
        self.S = {}
        suffix_to_method_mapping = {
            '.csv' : self.csv_to_json,
            '.json' : self.json_to_csv
        }
        suffix_to_method_mapping[self.path.suffix.lower()]()


    def json_to_csv(self):
        new_filename = self.filename_without_extension + '.csv'
        new_filepath = self.directory.joinpath(new_filename)
        counter = 1
